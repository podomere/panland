...
layout: research
ip_v4_address: "228.0.83.228"
date: 2022-11-19T16:17:19+00:00
draft: false
---

# Internet Addressing
On the Internet you might get a Public IPv4 address like ```228.0.83.228``` or an IPv6 address like ```2000:944d:7ee8:2175:fb1:7ed6:21f:da55```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```bd:ce:9c:93:01:ef```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://gerhold.info you initially access a DNS server to translate the host portion (gerhold) combined with the Top Level Domain (info) of the URL, to an IP address like ```227.229.190.188```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.7.106.20``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:fee0:93c8:8e64:618d%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {62.157.192.75, 34.123.119.194}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr bd:ce:9c:93:01:ef
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 95:da:2e:0c:e4:c7
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.13.8```, ```11.3.1```, or ```12.3.4```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
