...
layout: research
ip_v4_address: "178.243.68.203"
date: 2022-11-19T16:17:22+00:00
draft: false
---

# Internet Addressing
On the Internet you may have a Public IPv4 address like ```178.243.68.203``` or an IPv6 address like ```2000:4418:185a:5d2e:be84:538d:1dfc:7818```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```ee:46:6c:ae:bd:e4```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://stokes-sanford.co you initially access a DNS server to translate the host portion (stokes-sanford) combined with the Top Level Domain (co) of the URL, to an IP address like ```130.159.93.91```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.237.36.9``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:61c:ad6b:dbcd:c818%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {1.144.125.100, 4.15.229.119}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr ee:46:6c:ae:bd:e4
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 69:f5:0a:91:1f:96
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.11.2```, ```11.4.2```, or ```12.1.1```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
