...
layout: research
ip_v4_address: "126.9.51.13"
date: 2022-12-15T13:50:21+00:00
draft: false
---

# Internet Addressing
On the Internet you might get a Public IPv4 address like ```126.9.51.13``` or an IPv6 address like ```2000:3dff:a503:fddd:20a8:e329:b5de:43fa```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```bf:9e:91:d8:2d:51```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://corkery.name you initially access a DNS server to translate the host portion (corkery) combined with the Top Level Domain (name) of the URL, to an IP address like ```212.111.170.130```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT x.y; Win64; x64; rv:10.0) Gecko/20100101 Firefox/10.0```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.50.21.23``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:d6e6:7dc4:8b84:6aa4%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {140.162.196.46, 176.72.150.62}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr bf:9e:91:d8:2d:51
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 7c:e2:c8:1f:79:5e
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.13.5```, ```11.4.5```, or ```12.3.7```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
