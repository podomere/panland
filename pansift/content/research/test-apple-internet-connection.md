...
subtitle: "Ideate?"
layout: research
ip_v4_address: "94.242.162.152"
date: 2022-06-12T14:09:16+01:00
draft: false
---

# Internet Addressing
On the Internet you might have a Public IPv4 address like ```94.242.162.152``` or an IPv6 address like ```2000:1b2e:377:2ff:89a0:acec:3710:38cd```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```a5:ca:0a:dd:6c:7f```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://weimann.net you initially access a DNS server to translate the host portion (weimann) combined with the Top Level Domain (net) of the URL, to an IP address like ```29.253.229.188```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```172.19.191.44``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  172.19.191.44    UGScg  en0
...
<pre>
default   fe80:c421:1d2d:5a34:8b00%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {17.81.0.66, 89.19.250.253}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr a5:ca:0a:dd:6c:7f
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 2c:6d:f7:bf:8c:24
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.15.8```, ```11.3.6```, or ```12.0.8```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
