...
layout: research
ip_v4_address: "232.236.28.106"
date: 2023-02-04T16:26:22+00:00
draft: false
---

# Internet Addressing
On the Internet you may have a Public IPv4 address like ```232.236.28.106``` or an IPv6 address like ```2000:dbfb:3579:20df:71ea:70aa:193b:76e8```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```2f:6a:d2:f0:ad:81```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://reilly-bradtke.org you initially access a DNS server to translate the host portion (reilly-bradtke) combined with the Top Level Domain (org) of the URL, to an IP address like ```164.53.162.63```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```172.23.99.110``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  172.23.99.110    UGScg  en0
...
<pre>
default   fe80:b2b2:32ed:23e2:3af%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {78.208.64.21, 242.233.245.190}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr 2f:6a:d2:f0:ad:81
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 92:4a:7f:4c:bc:93
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.15.6```, ```11.2.8```, or ```12.1.4```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
