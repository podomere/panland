...
🚩 Basically though, an integer value of `10` doesn't map to a HEX value of `0x0480`, no sh*t! I racked my brain. Decimal `10` is `0xA` i.e. `A` in hex. Hex `0x0480` is `1152` in decimal (base `10`) as an integer. None of the networks or values lined up 🚩. 
