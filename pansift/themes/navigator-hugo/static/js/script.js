(function ($) { 
  ...
  if(document.getElementById("mapid") !== null){
    var mymap = null;
    var  mymap = L.map('mapid',{attributionControl:false, zoomControl: false, minZoom: 2, maxZoom: 5, zoom: 5}).setView([43, -41], 3);
    ...
    for (index = 0; index < champions_v4.length-70; index++) {
    ...
    function new_champion(lat,lng) {
      var popup = L.popup({closeButton:false})
        .setLatLng([lat, lng])
        .setContent('<a target="_blank" rel="nofollow" href="https://app.pansift.com/demo?first_insight=true" style="color: black;"><center><strong>Live Demo <i class="fas fa-external-link-alt"></i></strong></center></a>')
        .openOn(mymap);
      L.marker([lat,lng], {zIndexOffset: 10000}).addTo(mymap);
			// zin++;
    }
    var repetitions = 0;
