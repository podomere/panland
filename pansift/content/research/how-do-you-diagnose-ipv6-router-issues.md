...
layout: research
ip_v4_address: "94.123.64.154"
date: 2023-10-29T15:13:25+00:00
draft: false
---

## Internet Addressing
On the Internet you might have a Public IPv4 address like ```94.123.64.154``` or an IPv6 address like ```2000:e5ce:ee1f:89df:11e0:83dc:d6b8:8dc8```. You can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```7a:5f:4f:cf:96:dd```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

## Accessing the Web
To get to a web page like https://runolfsdottir-mcglynn.io you initially access a DNS server to translate the host portion (runolfsdottir-mcglynn) combined with the Top Level Domain (io) of the URL, to an IP address like ```145.36.133.8```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

## Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```192.0.0.36``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:61bb:25bf:47af:c347%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {145.88.39.76, 125.141.159.17}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr 7a:5f:4f:cf:96:dd
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 5c:a8:fc:65:b3:10
}</pre>
...
### Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.14.2```, ```11.4.9```, or ```12.2.9```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
