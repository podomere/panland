...
layout: research
ip_v4_address: "134.201.253.87"
date: 2023-10-29T15:13:18+00:00
draft: false
---

## Internet Addressing
On the Internet you may get a Public IPv4 address like ```134.201.253.87``` or an IPv6 address like ```2000:2780:b7d6:3fcb:c6:85ed:96b4:3a1e```. You can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```ca:ef:6d:0e:a1:8b```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

## Accessing the Web
To get to a web page like https://medhurst.name you initially access a DNS server to translate the host portion (medhurst) combined with the Top Level Domain (name) of the URL, to an IP address like ```91.206.34.70```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

## Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```172.31.56.58``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  172.31.56.58    UGScg  en0
...
<pre>
default   fe80:ba8e:491c:136c:c241%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {69.139.104.102, 238.248.224.91}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr ca:ef:6d:0e:a1:8b
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr c9:7f:6c:e0:a1:cc
}</pre>
...
### Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.14.8```, ```11.6.8```, or ```12.2.8```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
