...
subtitle: "Swag?"
layout: research
ip_v4_address: "25.242.53.3"
date: 2022-12-15T13:50:22+00:00
draft: false
---

# Internet Addressing
On the Internet you might get a Public IPv4 address like ```25.242.53.3``` or an IPv6 address like ```2000:a835:30af:5a91:dc3c:a712:14d4:aba3```. You can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```b1:83:bd:19:6c:a4```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://braun.info you initially access a DNS server to translate the host portion (braun) combined with the Top Level Domain (info) of the URL, to an IP address like ```113.241.225.111```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```192.168.134.94``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  192.168.134.94    UGScg  en0
...
<pre>
default   fe80:7206:75ce:6a9f:5202%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {117.229.74.38, 215.115.185.31}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr b1:83:bd:19:6c:a4
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 54:90:df:be:a0:8a
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.11.3```, ```11.5.9```, or ```12.2.2```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
