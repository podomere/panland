...
layout: research
ip_v4_address: "35.22.255.29"
date: 2023-10-06T17:10:43+01:00
draft: false
---

## Internet Addressing
On the Internet you may have a Public IPv4 address like ```35.22.255.29``` or an IPv6 address like ```2000:f5fc:9648:5bec:dee9:be43:3b3:5edf```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```ff:e2:0f:4c:6e:2f```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

## Accessing the Web
To get to a web page like https://lakin.name you initially access a DNS server to translate the host portion (lakin) combined with the Top Level Domain (name) of the URL, to an IP address like ```134.173.51.213```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (compatible; MSIE 9.0; AOL 9.7; AOLBuild 4343.19; Windows NT 6.1; WOW64; Trident/5.0; FunWebProducts)```

## Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.0.31.78``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:f52c:8d77:74fa:3947%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {197.96.149.50, 122.174.219.23}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr ff:e2:0f:4c:6e:2f
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr d6:e6:8e:fb:86:6a
}</pre>
...
### Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.12.6```, ```11.1.6```, or ```12.3.2```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
