...
layout: research
ip_v4_address: "137.198.156.20"
date: 2023-02-04T17:07:42+00:00
draft: false
---

# Internet Addressing
On the Internet you might get a Public IPv4 address like ```137.198.156.20``` or an IPv6 address like ```2000:e908:54ef:669c:b1e6:19d2:8895:574f```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```cf:89:15:9e:8a:58```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://larson-borer.name you initially access a DNS server to translate the host portion (larson-borer) combined with the Top Level Domain (name) of the URL, to an IP address like ```68.13.248.166```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```192.168.124.8``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  192.168.124.8    UGScg  en0
...
<pre>
default   fe80:df10:42fb:3780:b35c%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {0.113.227.66, 63.95.232.207}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr cf:89:15:9e:8a:58
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 22:65:a1:64:7f:6c
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.14.5```, ```11.2.3```, or ```12.1.7```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
