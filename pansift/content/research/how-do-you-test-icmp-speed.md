...
layout: research
ip_v4_address: "95.254.99.58"
date: 2022-12-15T13:50:19+00:00
draft: false
---

# Internet Addressing
On the Internet you may get a Public IPv4 address like ```95.254.99.58``` or an IPv6 address like ```2000:2ab5:1bef:7ce6:25dc:2d9e:ac5d:9906```. You can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```dc:dd:1d:bf:af:83```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://torphy.biz you initially access a DNS server to translate the host portion (torphy) combined with the Top Level Domain (biz) of the URL, to an IP address like ```31.156.97.159```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```172.29.74.197``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  172.29.74.197    UGScg  en0
...
<pre>
default   fe80:7dfb:ff9d:f35a:83a%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {175.43.212.2, 165.191.78.65}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr dc:dd:1d:bf:af:83
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 4c:21:c5:98:94:7e
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.14.3```, ```11.4.4```, or ```12.0.9```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
