...
layout: research
ip_v4_address: "252.2.241.81"
date: 2023-10-29T15:13:18+00:00
draft: false
---

## Internet Addressing
On the Internet you might have a Public IPv4 address like ```252.2.241.81``` or an IPv6 address like ```2000:6ff:c693:cb5c:84ca:7241:acab:970e```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```e4:fa:4c:6e:4d:d3```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

## Accessing the Web
To get to a web page like https://satterfield.name you initially access a DNS server to translate the host portion (satterfield) combined with the Top Level Domain (name) of the URL, to an IP address like ```197.199.40.120```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A```

## Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.219.202.90``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
default  10.219.202.90    UGScg  en0
...
<pre>
default   fe80:d0fe:751d:26f1:8fbd%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {228.252.181.76, 18.35.118.81}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr e4:fa:4c:6e:4d:d3
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr 7a:11:90:5e:30:9e
}</pre>
...
### Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.13.3```, ```11.2.1```, or ```12.1.2```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
