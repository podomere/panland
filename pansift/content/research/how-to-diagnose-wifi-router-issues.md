...
layout: research
ip_v4_address: "167.240.143.55"
date: 2022-05-05T14:28:25+01:00
draft: false
---

# Internet Addressing
On the Internet you may have a Public IPv4 address like ```167.240.143.55``` or an IPv6 address like ```2000:5357:7337:941c:af59:e5c5:adab:f0b0```. We can check this from [https://test-ipv6.com/](https://test-ipv6.com/). Yet, for 'non-techies' to try and communicate these addresses, or even call out MAC addresses like ```fb:b3:4b:a5:3c:cd```, it can be error prone and gets complicated quickly. Additionally, this doesn't give you any historical data (especially back when previous problems occured).

# Accessing the Web
To get to a web page like https://ledner.io you initially access a DNS server to translate the host portion (ledner) combined with the Top Level Domain (io) of the URL, to an IP address like ```3.13.189.216```. Your computer and browser actually sends its type with all web requests e.g. <br>```Mozilla/5.0 (Windows NT x.y; Win64; x64; rv:10.0) Gecko/20100101 Firefox/10.0```

# Default Gateways
Your default gateway is normally an automatically configured address via DHCP. You get a default gateway like ```10.4.108.61``` (though they normally end in .1 or .254 depending upon the scope size) and this is where your computer sends all its traffic to be routed onwards. For ```IPv6``` we have a deep dive on [how-to-fix-ipv6-connectivity/](/blog/how-to-fix-ipv6-connectivity/) but you can check on Mac or Linux with:
...
<pre>
default   fe80:67c6:8d18:cafe:fa84%en0  UGcg   en0
...
<pre>
...
domain_name_server (ip_mult): {33.200.1.83, 173.241.133.188}
end (none):
...</pre>
...
Options[4] = {
  CLIENTID (1) Length 14: DUID LLT HW 1 Time 668691856 Addr fb:b3:4b:a5:3c:cd
  DNS_SERVERS (23) Length 32: 2606:4700:4700::1111, 2001:4860:4860::8844
  DOMAIN_LIST (24) Length 0:  Invalid
  SERVERID (2) Length 10: DUID LL HW 1 Addr c7:fa:ae:1b:15:cf
}</pre>
...
## Apple macOS / OSX
No matter what version of OSX/macOS you are on, ```10.13.9```, ```11.2.6```, or ```12.1.4```, there are a range of tools for troubleshooting. Unfortunately, between these manual actions and scripts, they don't give you a series of correlated values over time. This is where automated remote troubleshooting comes in to its own, especially for teams that embrace remote work and Work From Anywhere (WFA).
